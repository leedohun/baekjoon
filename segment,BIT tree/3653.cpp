...
int segment_init(int start, int end, int index) {
    if (start == end) {
        if (M + 1 <= start) return tree[index] = 1;
        else return tree[index] = 0;
    }
    int mid = (start + end) / 2;
    return tree[index]
        = segment_init(start, mid, index * 2)
...
int query(int start, int end, int index, int left, int right) {
    ...
    int mid = (start + end) / 2;
    return query(start, mid, index * 2, left, right)
...
void solved() {
    ...
    while (t--) {
        ...
        int i = M;        
        segment_init(1, M + N, 1);
        for (int k = 0; k < M; k++) {
            int tmp; cin >> tmp;
            cout << query(1, M + N, 1, 1, m[tmp]) - 1 << " ";

            segment_update(1, M + N, 1, m[tmp], 0);
            m[tmp] = i--;
            segment_update(1, M + N, 1, m[tmp], 1);
        }
        cout << "\n";
    }
}
