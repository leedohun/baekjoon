...
int segment_init(int start, int end, int index) {
    if (start == end) {
        tree[index] = start;
    ...
    return tree[index] = minIndex(segment_init(start, mid, index * 2),
        segment_init(mid + 1, end, index * 2 + 1));
}

int segment_update(int start, int end, int index, int target) {
    if (target < start || end < target || start == end) return tree[index];
    
    int mid = (start + end) / 2;
    return tree[index] = minIndex(segment_update(start, mid, index * 2, target),
        segment_update(mid + 1, end, index * 2 + 1, target));
}

int query(int start, int end, int index, int left, int right) {
    if (left > end || right < start) return tree[0];
    if (left <= start && end <= right) return tree[index];
    
    int mid = (start + end) / 2;
    return minIndex(query(start, mid, index * 2, left, right),
        query(mid + 1, end, index * 2 + 1, left, right));
}
