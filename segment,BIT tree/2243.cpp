...
int query(int start, int end, int index, int target) {
    ...
    if (target <= tree[index * 2])
        ...
        return query(mid + 1, end, index * 2 + 1, target - tree[index * 2]);
}
...
void solved() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (A == 2) {
            int B, C; cin >> B >> C;
            update(1, 1000001, 1, B, C); //시작, 끝, 인덱스 B의 원소를 C만큼 수정
        }
        else {
            int B; cin >> B;
            int tmp = query(1, 1000001, 1, B);
            cout << tmp << "\n";
            update(1, 1000001, 1, tmp, -1);
        }
    }
}
