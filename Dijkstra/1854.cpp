...
void solved() {
    ...
    while (!pq.empty()) {
        ...
        for (int i = 0; i < v[now.second].size(); i++) {
            pair<int, int> next = v[now.second][i];

            if (dist[next.first].size() < K) {
                dist[next.first].push(now.first + next.second);
                pq.push({ now.first + next.second, next.first });
            }
            else if(dist[next.first].size() == K 
                && dist[next.first].top() > now.first + next.second) {
                dist[next.first].pop();
                dist[next.first].push(now.first + next.second);
                pq.push({ now.first + next.second, next.first });
            }
